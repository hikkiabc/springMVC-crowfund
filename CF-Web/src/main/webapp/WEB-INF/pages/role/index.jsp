...
<head>
    ...
    <style type="text/css">
        ...
        .activePage{
            color: rgba(255, 48, 161, 0.54) !important;
...
<%--                </c:if>--%>
<%--                <c:if test="${page.pageNum!=1}">--%>
<%--                    <li class="page-item "><a href="${contextPaht}/user/queryRolesByPage?pageNum=${page.pageNum-1}">Previous</a></li>--%>
<%--                </c:if>--%>
<%--                <c:forEach begin="1" end="${page.totalPages}" var="num">--%>
<%--                    <li--%>
<%--                            <c:if test="${num==page.pageNum}">--%>
<%--                                    class="active"--%>
<%--                            </c:if>class="page-item"><a href="${contextPaht}/user/queryRolesByPage?pageNum=${num}" class="page-link">${num}</a></li>--%>
<%--                </c:forEach>--%>
<%--                <c:if test="${page.pageNum>=page.totalPages}">--%>
<%--                    <li class="page-item disabled"><a href="#">Next</a></li>--%>
<%--                </c:if>--%>
<%--                <c:if test="${page.pageNum<page.totalPages}">--%>
<%--                    <li class="page-item "><a href="${contextPaht}/user/queryRolesByPage?pageNum=${page.pageNum+1}">Next</a></li>--%>
<%--                </c:if>--%>
...
<script>
    ...
    function queryRolesByPage(pageNum,pageSize) {

        let keywords=  $("#searchInput").val()
        $.ajax({
            url:'${contextPath}/role/queryRolesByPage',
            ...
            success:(res)=>{
                ...
                let contentMain=''
                            res.res.data.forEach((i,index)=>{
                                contentMain+='                                <tr>\n' +
                                    ...
                                    '                                    <td class=\'td-btn\'>\n' +
                                    '                                        <a href="javascript:void(0)" onclick="window.location.href=\'${contextPath}/role/toAssignPermission?roleId='+i.roleId+'\'" data-toggle="modal"><i class="material-icons" data-toggle="tooltip" title="assignRole">&#xE872;</i></a>\n' +
                ...
                if(res.res.pageNum<2){
                    ...
                    content+='<li  class="page-item "><a onclick="queryRolesByPage('+(res.res.pageNum-1)+')" href="javascript:void(0)">Previous</a></li>'
                }

                for(let i=1;i<res.res.totalPages+1;i++){
                    if(res.res.pageNum==i){
                        content+= '<li onclick="queryRolesByPage('+i+')" class="page-item "><a class="activePage" href="javascript:void(0)">'+i+'</a></li>'

                    }else
                    content+= '<li onclick="queryRolesByPage('+i+')" class="page-item "><a  href="javascript:void(0)">'+i+'</a></li>'
                }

                if(res.res.pageNum>=res.res.totalPages){
                    content+='<li class="page-item disabled"><a>Next</a></li>'
                }else{
                    content+='<li  class="page-item "><a onclick="queryRolesByPage('+(res.res.pageNum+1)+')" href="javascript:void(0)">Next</a></li>'
                }
