...
<div class="wrapper">
    ...
    <div class="main-panel">
        ...
        </nav> <button id="submit" class="btn btn-primary">Submit Assignment</button>
...
var obj=$("#"+treeNode.tId+"_a")
            ...
            async:{
              enable:true,
                url:'${contextPath}/role/queryPermissions?roleId=${param.roleId}'
            },
            callback: {
       ...
       let zTree= $.fn.zTree.init($("#tree"), setting);
        $("#submit").click(function () {
            // console.log(1)

                let checkedNodes = zTree.getCheckedNodes(true);
            let idList = checkedNodes.reduce((pre, curr)=>{

                pre.push(curr.id)
               return      pre
            },[]);
           $.ajax({
               url:'${contextPath}/role/updatePermission',
               method:'post',
               contentType:'application/json;charset=UTF-8',

               data:JSON.stringify({idList,roleId:${param.roleId}}) ,
               success(res){
                  if(res.res>=0){
                      <%--window.location.href='${contextPath}/role/queryPermissions?roleId=${param.roleId}'--%>
                      location.reload()
                  }
               },
               error(e){
    ...
    function addDiyDom(treeId, treeNode) {
        ...
        if(treeNode.icon){
            aObj.removeClass("button ico_docu ico_open").addClass(treeNode.icon)
